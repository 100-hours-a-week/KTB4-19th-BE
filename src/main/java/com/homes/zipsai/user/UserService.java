...
public class UserService {
    ...
    public UserService(UserRepository users, TokenService tokens) { this.users = users; this.tokens = tokens; }
    ...
    public Map<String, Object> patch(AuthPrincipal principal, JsonNode body) {
        ...
        if (body.has("userName")) { user.setUserName(UserInput.name(UserInput.text(body, "userName", false))); result.put("userName", user.getUserName()); }
        if (body.has("phone")) { user.setPhone(UserInput.phone(UserInput.text(body, "phone", false))); result.put("phone", user.getPhone()); }
        ...
        if (body.has("agreements")) {
            for (var entry : updates.entrySet()) {
                ...
                if (!unchanged) user.agree(entry.getKey(), entry.getValue());
            }
