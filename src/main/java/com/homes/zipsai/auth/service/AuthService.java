...
public class AuthService {
    ...
    public Long signup(SignupRequest request) {
        ...
        if (!password.equals(request.passwordConfirm())) {
            throw UserInput.invalid("passwordConfirm", Reason.PASSWORD_CONFIRMATION_MISMATCH);
        }
    ...
    public Tokens login(LoginRequest loginRequest) {
        ...
        String passwordHash = user == null ? dummyHash : user.getPassword();
        ...
        return tx.execute(status -> {
            ...
            data.put("accessToken", tokens.access(user));
    ...
    public Tokens reissue(String raw) {
        ...
        return tx.execute(status -> {
            ...
            Map<String, Object> data = Map.of(
                    "accessToken", tokens.access(user),
                    "tokenType", "Bearer"
            );
    ...
    public void logout(AuthPrincipal principal, String rawRefreshToken) {
        ...
        tx.executeWithoutResult(status -> {
            RefreshSession session = sessions.findLockedByHash(TokenService.hash(rawRefreshToken))
                    .orElseThrow(UnauthorizedException::new);
